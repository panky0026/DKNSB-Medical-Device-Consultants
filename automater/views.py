...
def home(request):
    if request.method=='POST' and request.FILES:
        ...
        if l[1]=='xlsx' or l[1]=='xls':
            ...
            df1=pd.read_excel('files/COAdata.xlsx')
    ...
    #a way to access the context2 with name of material in context
            ...
            for i in range(len(c1)):
                ...
                if os.path.exists(f"files/sign/{c7[i]}.png"):
                    img_d=f"files/sign/{c7[i]}.png"
                else:
                    img_d=f"files/sign/random.png"
                ...
                df2=pd.read_excel('files/COAdata.xlsx',sheet_name='sampling plan')
                mini=df2['min'].values 
                maxi=df2['max'].values
                ...
                doc.save(f'output/COAfiles/COA{c1[i]}.docx')
...
def download(request):
    ...
    for fpath in filenames:
        ...
        if zip_path.split('.')[1]=='docx':
            zf.write(fpath, f'/COAs/{zip_path}')
        else:
